...
func TestCopy(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Install: Too many input arguments",
			argv: []string{"mender-artifact", "install", "-m", "-0600", "foo", ":/etc/mender/test.txt", "bar"},
			err:  "got 3 arguments, wants two",
		},
		{
			name: "Install: No artifact or sdimg provided",
			argv: []string{"mender-artifact", "install", "-m", "0600", "foo", "bar"},
